...
MARKET_FEISHU_TO="${KCLAW_CRON_FEISHU_MARKET_TO:-${KCLAW_CRON_FEISHU_TO:-}}"
...
KCLAW_CRON_FEISHU_TO remains a legacy fallback only when every job should share
one target.
...
add_agent_job "board index daily refresh" "0 9 * * 1-6" "pattern-strategy" 1800 "off" "$board_index_message" "$MARKET_FEISHU_TO"
